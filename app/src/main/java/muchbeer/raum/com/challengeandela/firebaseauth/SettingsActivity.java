...
import muchbeer.raum.com.challengeandela.utility.ChangePhotoDialog;
import muchbeer.raum.com.challengeandela.utility.FilePaths;
import muchbeer.raum.com.challengeandela.utility.FirebaseUtil;

public class SettingsActivity extends AppCompatActivity implements OnPhotoReceivedListener{
...
Toast.makeText(getApplicationContext(), "Granted permission now go into phot", Toast.LENGTH_LONG).show();
                }else{
                    verifyStoragePermissions();

                    Toast.makeText(getApplicationContext(), "Asking for permission", Toast.LENGTH_LONG).show();
                }
...
//sort ImageLoader init Issue
    ...
    private void getUserAccountsData(){
        ...
        Query query1 = reference.child(getString(R.string.dbnode_users))
                .orderByKey()
               // .orderByValue()  This is when you want to search a certain value from the database
               // .limitToFirst(5) Limit to the first 5 users
        ...
        query1.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(getApplicationContext(), "Fail to retrieve the data", Toast.LENGTH_SHORT).show();
            }
        });
            ...
            public void onClick(View v) {
                ...
                if(!mName.getText().toString().equals("")){
                    reference.child(getString(R.string.dbnode_users))
                            ...
                            .setValue(mName.getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                Log.d(TAG, "onComplete: Edit saved is complete.");
                                Toast.makeText(SettingsActivity.this, "Edit was successful",
                                        Toast.LENGTH_SHORT).show();

                                //Launch MainActivity
                                Intent mainActivity = new Intent(SettingsActivity.this, SignedInActivity.class);
                                startActivity(mainActivity);
                                finish();
                            }else{
                                Log.d(TAG, "onComplete: No edit saved.");
                                Toast.makeText(SettingsActivity.this, "onComplete: No edit saved.",
                                        Toast.LENGTH_SHORT).show();
...
//specify where the photo will be stored
        ...
        /*final StorageReference firememeRef = FirebaseUtil.mStorageRef
        ...
        final StorageReference firememeRef = FirebaseStorage.getInstance().getReference()
                .child(filePaths.FIREBASE_IMAGE_STORAGE + "/" + FirebaseAuth.getInstance().getCurrentUser().getUid()
                        + "/profile_image"); //just replace the old image with the new one
        ...
        if(mBytes.length/MB < MB_THRESHHOLD) {
            ...
            uploadTask = firememeRef.putBytes(mBytes, metadata);
